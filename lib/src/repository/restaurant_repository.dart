...
Future<Stream<Review>> getRestaurantReviews(String id) async {
  final String url = '${apiBaseUrl}restaurant_reviews?with=user&search=restaurant_id:$id';
  try {
    final client = new http.Client();
...
Future<Stream<Review>> getRecentReviews() async {
  final String url = '${apiBaseUrl}restaurant_reviews?orderBy=updated_at&sortedBy=desc&limit=3&with=user';
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', Uri.parse(url)));
    return streamedRest.stream.transform(utf8.decoder).transform(json.decoder).map((data) => Helper.getData(data)).expand((data) => (data as List)).map((data) {
...
Future<Review> addRestaurantReview(Review review, Restaurant restaurant) async {
  final String url = '${apiBaseUrl}restaurant_reviews';
  final client = new http.Client();
